...
# Stash any local changes before fetching
git stash push -q -m "Auto-stash before fetching new version" || true

git fetch --depth 1 origin "${MS_COMMIT}"
git checkout FETCH_HEAD

# Don't pop the stash - we want fresh Microsoft source
